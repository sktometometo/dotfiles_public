...
起動後、セッション情報は `~/.cache/agent-tools/chrome-sessions/` に JSON で保存される。
同じ CDP ポートに Chrome が残っていれば、`chrome-app-start.sh` の再実行時は既存セッションを再利用する。
...
- Keep / Teams / freee のような常用サービスは、専用プロファイル・専用 CDP ポートで分離する
